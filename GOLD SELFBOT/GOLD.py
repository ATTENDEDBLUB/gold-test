...
import ctypes
ctypes.windll.kernel32.SetConsoleTitleW("ATTENDEDBLUB")
import aiohttp
import colorama
import discord
import numpy
...
stream_url = "https://www.twitch.tv/Gold"
tts_language = "en"
...
def startprint():
    ...
    print(f'''{Fore.RESET}
                  ...
                  {Fore.YELLOW}[{Fore.RESET}User{Fore.YELLOW}] {Fore.WHITE}{Gold.user.name}#{Gold.user.discriminator}  {Fore.YELLOW}[{Fore.RESET}Sniper{Fore.YELLOW}] {Fore.WHITE}{nitro}
                  {Fore.YELLOW}[{Fore.RESET}Prefix{Fore.YELLOW}] {Fore.WHITE}{Gold.command_prefix}       {Fore.YELLOW}[{Fore.RESET}Note{Fore.YELLOW}] {Fore.WHITE}Fuck TOS
...
def Init():
    token = config.get('token')
    try:
        Gold.run(token, bot=False, reconnect=True)
        os.system(f'title (Gold Selfbot) - Version {SELFBOT.__version__}')
    except discord.errors.LoginFailure:
        print(f"{Fore.YELLOW}[ERROR] {Fore.YELLOW}Invalid token" + Fore.RESET)
        os.system('pause >NUL')
...
colorama.init()
Gold = discord.Client()
Gold = commands.Bot(description='Gold Selfbot', command_prefix=prefix, self_bot=True)

Gold.antiraid = False
Gold.msgsniper = True
Gold.slotbot_sniper = True
Gold.giveaway_sniper = True
Gold.mee6 = False
Gold.mee6_channel = None
Gold.yui_kiss_user = None
Gold.yui_kiss_channel = None
Gold.yui_hug_user = None
Gold.yui_hug_channel = None
Gold.sniped_message_dict = {}
Gold.sniped_edited_message_dict = {}
Gold.whitelisted_users = {}
Gold.copycat = None
Gold.remove_command('help')
...
async def on_message_edit(before, after):
    await Gold.process_commands(after)


@Gold.event
async def on_message(message):
    if Gold.copycat is not None and Gold.copycat.id == message.author.id:
        await message.channel.send(chr(173) + message.content)

    def GiveawayData():
        print(
            f"{Fore.YELLOW} - CHANNEL: {Fore.WHITE}[{message.channel}]"
            f"\n{Fore.YELLOW} - SERVER: {Fore.WHITE}[{message.guild}]"
            + Fore.RESET)
    ...
    def NitroData(elapsed, code):
        print(
            f"{Fore.YELLOW} - CHANNEL: {Fore.WHITE}[{message.channel}]"
            f"\n{Fore.YELLOW} - SERVER: {Fore.WHITE}[{message.guild}]"
            f"\n{Fore.YELLOW} - AUTHOR: {Fore.WHITE}[{message.author}]"
            f"\n{Fore.YELLOW} - ELAPSED: {Fore.WHITE}[{elapsed}]"
            f"\n{Fore.YELLOW} - CODE: {Fore.WHITE}{code}"
            + Fore.RESET)
    ...
    if 'discord.gift/' in message.content:
        if nitro_sniper:
            ...
            if 'This gift has been redeemed already.' in r:
                print(""
                      f"\n{Fore.YELLOW}[{time} - Invalid Nitro Sniped]" + Fore.RESET)
                NitroData(elapsed, code)

            elif 'subscription_plan' in r:
                print(""
                      f"\n{Fore.YELLOW}[{time} - Nitro Sniped]" + Fore.RESET)
                NitroData(elapsed, code)
    ...
    if 'Someone just dropped' in message.content:
        if Gold.slotbot_sniper:
            if message.author.id == 346353957029019648:
                try:
                    await message.channel.send('~grab')
                except discord.errors.Forbidden:
                    print(""
                          f"\n{Fore.YELLOW}[{time} - SlotBot Couldnt Grab]" + Fore.RESET)
                    SlotBotData()
                print(""
                      f"\n{Fore.YELLOW}[{time} - Slotbot Grabbed]" + Fore.RESET)
                SlotBotData()
        else:
            return

    if 'GIVEAWAY' in message.content:
        if Gold.giveaway_sniper:
            if message.author.id == 294882584201003009:
                try:
                    await message.add_reaction("🎉")
                except discord.errors.Forbidden:
                    print(""
                          f"\n{Fore.YELLOW}[{time} - Invalid Giveaway Sniped]" + Fore.RESET)
                    GiveawayData()
                print(""
                      f"\n{Fore.YELLOW}[{time} - Giveaway Sniped]" + Fore.RESET)
                GiveawayData()
        else:
            return

    if f'Congratulations <@{Gold.user.id}>' in message.content:
        if Gold.giveaway_sniper:
            if message.author.id == 294882584201003009:
                print(""
                      f"\n{Fore.YELLOW}[{time} - Giveaway Won]" + Fore.RESET)
                GiveawayData()
        else:
            return

    await Gold.process_commands(message)


@Gold.event
async def on_member_ban(guild: discord.Guild, user: discord.user):
    if Gold.antiraid is True:
        try:
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.ban):
                if guild.id in Gold.whitelisted_users.keys() and i.user.id in Gold.whitelisted_users[
                    guild.id].keys() and i.user.id is not Gold.user.id:
                    print("not banned - " + i.user.name)
                else:
                    print("banned - " + i.user.name)
                    await guild.ban(i.user, reason="Gold Anti-Nuke")
...
async def on_member_join(member):
    if Gold.antiraid is True and member.bot:
        try:
            guild = member.guild
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.bot_add):
                if member.guild.id in Gold.whitelisted_users.keys() and i.user.id in Gold.whitelisted_users[
                    member.guild.id].keys():
                    return
                else:
                    await guild.ban(member, reason="Gold Anti-Nuke")
                    await guild.ban(i.user, reason="Gold Anti-Nuke")
        except Exception as e:
            print(e)
...
@Gold.command(aliases=['wl'])
async def whitelist(ctx, user: discord.Member = None):
    await ctx.message.delete()
    if user is None:
        await ctx.send("Please specify a user to whitelist")
    else:
        if ctx.guild.id not in Gold.whitelisted_users.keys():
            Gold.whitelisted_users[ctx.guild.id] = {}
        if user.id in Gold.whitelisted_users[ctx.guild.id]:
            await ctx.send('That user is already whitelisted')
        else:
            Gold.whitelisted_users[ctx.guild.id][user.id] = 0
            await ctx.send("Whitelisted **" + user.name.replace("*", "\*").replace("`", "\`").replace("_",
                                                                                                      "\_") + "#" + user.discriminator + "**")
    # else:
    #     user = Gold.get_user(id)
    #     if user is None:
...
@Gold.command(aliases=['wld'])
async def whitelisted(ctx, g=None):
    await ctx.message.delete()
    if g == '-g' or g == '-global':
        whitelist = '`All Whitelisted Users:`\n'
        for key in Gold.whitelisted_users:
            for key2 in Gold.whitelisted_users[key]:
                user = Gold.get_user(key2)
                whitelist += '**+ ' + user.name.replace('*', "\*").replace('`', "\`").replace('_',
                                                                                              "\_") + "#" + user.discriminator + "** - " + Gold.get_guild(
        ...
        for key in Gold.whitelisted_users:
            if key == ctx.guild.id:
                for key2 in Gold.whitelisted_users[ctx.guild.id]:
                    user = Gold.get_user(key2)
                    whitelist += '**+ ' + user.name.replace('*', "\*").replace('`', "\`").replace('_',
...
async def unwhitelist(ctx, user: discord.Member = None):
    if user is None:
        ...
        if ctx.guild.id not in Gold.whitelisted_users.keys():
            ...
            return
        if user.id in Gold.whitelisted_users[ctx.guild.id]:
            Gold.whitelisted_users[ctx.guild.id].pop(user.id, 0)
            user2 = Gold.get_user(user.id)
            await ctx.send(
                'Successfully unwhitelisted **' + user2.name.replace('*', "\*").replace('`', "\`").replace('_',
                                                                                                           "\_") + '#' + user2.discriminator + '**')


@Gold.command(aliases=['clearwl', 'clearwld'])
async def clearwhitelist(ctx):
    await ctx.message.delete()
    Gold.whitelisted_users.clear()
...
async def yuikiss(ctx, user: discord.User = None):
    ...
    if isinstance(ctx.message.channel, discord.DMChannel) or isinstance(ctx.message.channel, discord.GroupChannel):
        ...
        if user is None:
            ...
            return
        Gold.yui_kiss_user = user.id
        Gold.yui_kiss_channel = ctx.channel.id
        if Gold.yui_kiss_user is None or Gold.yui_kiss_channel is None:
            await ctx.send('An impossible error occured, try again later or contact swag')
            return
        while Gold.yui_kiss_user is not None and Gold.yui_kiss_channel is not None:
            await Gold.get_channel(Gold.yui_kiss_channel).send('yui kiss ' + str(Gold.yui_kiss_user), delete_after=0.1)
...
async def yuihug(ctx, user: discord.User = None):
    ...
    if isinstance(ctx.message.channel, discord.DMChannel) or isinstance(ctx.message.channel, discord.GroupChannel):
        ...
        if user is None:
            ...
            return
        Gold.yui_hug_user = user.id
        Gold.yui_hug_channel = ctx.channel.id
        if Gold.yui_hug_user is None or Gold.yui_hug_channel is None:
            await ctx.send('An impossible error occured, try again later or contact swag')
            return
        while Gold.yui_hug_user is not None and Gold.yui_hug_channel is not None:
            await Gold.get_channel(Gold.yui_hug_channel).send('yui hug ' + str(Gold.yui_hug_user), delete_after=0.1)
            await asyncio.sleep(60)
...
async def mee6(ctx, param=None):
    ...
    while Gold.mee6 is True:
        ...
        await Gold.get_channel(Gold.mee6_channel).send(random.choice(sentences), delete_after=0.1)
        await asyncio.sleep(60)


@Gold.command(aliases=['slotsniper', "slotbotsniper"])
async def slotbot(ctx, param=None):
    await ctx.message.delete()
    Gold.slotbot_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        Gold.slotbot_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        Gold.slotbot_sniper = False
...
@Gold.command(aliases=['giveawaysniper'])
async def giveaway(ctx, param=None):
    await ctx.message.delete()
    Gold.giveaway_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        Gold.giveaway_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        Gold.giveaway_sniper = False
...
async def on_message_delete(message):
    if message.author.id == Gold.user.id:
        return
    if Gold.msgsniper:
        if isinstance(message.channel, discord.DMChannel) or isinstance(message.channel, discord.GroupChannel):
            ...
            if len(attachments) == 0:
                ...
                message_content = "`" + str(
                    ...
                    message.content) + "\n\n**Attachments:**\n" + links
                await message.channel.send(message_content)
    if len(Gold.sniped_message_dict) > 1000:
        Gold.sniped_message_dict.clear()
    attachments = message.attachments
    if len(attachments) == 0:
        channel_id = message.channel.id
        message_content = "`" + str(discord.utils.escape_markdown(str(message.author))) + "`: " + str(
            message.content).replace("@everyone", "@\u200beveryone").replace("@here", "@\u200bhere")
        Gold.sniped_message_dict.update({channel_id: message_content})
    else:
        links = ""
        for attachment in attachments:
            links += attachment.proxy_url + "\n"
...
@Gold.event
async def on_message_edit(before, after):
    if before.author.id == Gold.user.id:
        return
    if Gold.msgsniper:
        ...
        if isinstance(before.channel, discord.DMChannel) or isinstance(before.channel, discord.GroupChannel):
            ...
            if len(attachments) == 0:
                ...
                message_content = "`" + str(
                    ...
                    before.content) + "\n\n**Attachments:**\n" + links
                await before.channel.send(message_content)
    if len(Gold.sniped_edited_message_dict) > 1000:
        Gold.sniped_edited_message_dict.clear()
    attachments = before.attachments
    if len(attachments) == 0:
        ...
        message_content = "`" + str(discord.utils.escape_markdown(str(before.author))) + "`: \n**BEFORE**\n" + str(
            before.content).replace("@everyone", "@\u200beveryone").replace("@here",
                                                                            "@\u200bhere") + "\n**AFTER**\n" + str(
            after.content).replace("@everyone", "@\u200beveryone").replace("@here", "@\u200bhere")
        Gold.sniped_edited_message_dict.update({channel_id: message_content})
    else:
        links = ""
        for attachment in attachments:
            links += attachment.proxy_url + "\n"
...
async def snipe(ctx):
    ...
    if currentChannel in Gold.sniped_message_dict:
        await ctx.send(Gold.sniped_message_dict[currentChannel])
    else:
        await ctx.send("No message to snipe!")


@Gold.command(aliases=["esnipe"])
async def editsnipe(ctx):
    await ctx.message.delete()
    currentChannel = ctx.channel.id
    if currentChannel in Gold.sniped_edited_message_dict:
        await ctx.send(Gold.sniped_edited_message_dict[currentChannel])
    else:
        await ctx.send("No message to snipe!")
...
@Gold.command(aliases=["img", "searchimg", "searchimage", "imagesearch", "imgsearch"])
    ...
    if str(image_tags[2]['src']).find("https://trkn.us/pixel/imp/c="):
        ...
        try:
            ...
            with io.BytesIO(image) as file:
                await ctx.send(f"Search result for: **{args}**", file=discord.File(file, f"Gold_anal.png"))
        except:
...
@Gold.command(aliases=["addemoji", "stealemote", "addemote"])
...
@Gold.command(aliases=["stopcopycatuser", "stopcopyuser", "stopcopy"])
async def stopcopycat(ctx):
    await ctx.message.delete()
    if Gold.user is None:
        await ctx.send("You weren't copying anyone to begin with")
        return
    await ctx.send("Stopped copying " + str(Gold.copycat))
    Gold.copycat = None


@Gold.command(aliases=["copycatuser", "copyuser"])
async def copycat(ctx, user: discord.User):
    await ctx.message.delete()
    Gold.copycat = user
    await ctx.send("Now copying " + str(Gold.copycat))


@Gold.command(aliases=["9/11", "911", "terrorist"])
...
@Gold.command(aliases=["jerkoff", "ejaculate", "orgasm"])
async def cum(ctx):
    await ctx.message.delete()
...
@Gold.command(aliases=["spamchangegcname", "changegcname"])
...
@Gold.command(aliases=['geolocate', 'iptogeo', 'iptolocation', 'ip2geo', 'ip'])
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(str(res['message'])) as resp:
                        image = await resp.read()
                with io.BytesIO(image) as file:
                    await ctx.send(file=discord.File(file, f"Gold_tweet.png"))
            except:
                await ctx.send(res['message'])


@Gold.command(aliases=["distort"])
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(str(res['message'])) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Gold_magik.png"))
        except:
            await ctx.send(res['message'])
    else:
...
@Gold.command(aliases=['markasread', 'ack'])
async def read(ctx):
    await ctx.message.delete()
    for guild in Gold.guilds:
        await guild.ack()
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(str(res['message'])) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Gold_fry.png"))
        except:
            await ctx.send(res['message'])
    else:
        avatar = str(user.avatar_url_as(format="png"))
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(endpoint) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Gold_blur.png"))
        except:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Gold_supreme.png"))
    except:
        await ctx.send(endpoint)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Gold_dark_supreme.png"))
    except:
        await ctx.send(endpoint)


@Gold.command(aliases=["facts"])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Gold_facts.png"))
    except:
        await ctx.send(endpoint)


@Gold.command(aliases=["blurp"])
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(str(res['message'])) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Gold_blurpify.png"))
        except:
            await ctx.send(res['message'])
    else:
        avatar = str(user.avatar_url_as(format="png"))
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(endpoint) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Gold_invert.png"))
        except:
            await ctx.send(endpoint)
    else:
        avatar = str(user.avatar_url_as(format="png"))
...
@Gold.command(aliases=["pornhublogo", "phlogo"])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Gold_pornhub_logo.png"))
    except:
        await ctx.send(endpoint)


@Gold.command(aliases=["pornhubcomment", 'phc'])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res["message"]) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Gold_pornhub_comment.png"))
    except:
        await ctx.send(res["message"])
...
@Gold.command(aliases=["reversesearch", "anticatfish", "catfish"])
...
@Gold.command(aliases=['pfp', 'avatar'])
...
@Gold.command(aliases=["del", "quickdel"])
...
@Gold.command(name='1337speak', aliases=['leetspeak'])
...
async def ghost(ctx):
    ...
    if config.get('password') == 'password-here':
        print(f"{Fore.YELLOW}[ERROR] {Fore.YELLOW}You didnt put your password in the config.json file" + Fore.RESET)
    else:
        password = config.get('password')
        with open('Images/Avatars/Transparent.png', 'rb') as f:
            try:
                await Gold.user.edit(password=password, username="ٴٴٴٴ", avatar=f.read())
            except discord.HTTPException as e:
                print(f"{Fore.YELLOW}[ERROR]: {Fore.YELLOW}{e}" + Fore.RESET)


@Gold.command(aliases=['pfpget', 'stealpfp'])
    ...
    if config.get('password') == 'password-here':
        ...
        try:
            ...
            with open('Images/Avatars/Stolen/Stolen.png', 'rb') as f:
                await Gold.user.edit(password=password, avatar=f.read())
        except discord.HTTPException as e:
            print(f"{Fore.YELLOW}[ERROR]: {Fore.YELLOW}{e}" + Fore.RESET)


@Gold.command(name='set-pfp', aliases=['setpfp', 'pfpset,"changepfp'])
...
@Gold.command(aliases=['wouldyourather', 'would-you-rather', 'wyrq'])
...
@Gold.command(aliases=['dong', 'penis'])
...
@Gold.command(aliases=['changehypesquad'])
...
@Gold.command(aliases=['tokenfucker', 'disable', 'crash'])
...
@Gold.command(aliases=['fakeconnection', 'spoofconnection', 'spoofcon', "fakecon"])
async def fakenet(ctx, _type=None, *, name=None):
    ...
    if r.status_code == 200:
        ...
        await ctx.send(
            '**[ERROR]** `Gold Fake-Connection doesn\'t work anymore because Discord patched connection-spoofing`')


@Gold.command(aliases=['tokinfo', 'tdox'])
...
@Gold.command(aliases=["copyguild", "copyserver"])
async def copy(ctx):  # b'\xfc'
    await ctx.message.delete()
    await Gold.create_guild(f'backup-{ctx.guild.name}')
    await asyncio.sleep(4)
    for g in Gold.guilds:
...
@Gold.command(aliases=["rekt", "nuke"])
async def destroy(ctx):
    ...
    try:
        ...
        pass
    for _i in range(250):
        await ctx.guild.create_text_channel(name="Gold")
    for _i in range(250):
        await ctx.guild.create_role(name="Gold", color=RandomColor())


@Gold.command(aliases=["banwave", "banall", "etb"])
async def massban(ctx):
    await ctx.message.delete()
    users = list(ctx.guild.members)
    for user in users:
        try:
            await user.ban(reason="Gold")
        except:
            pass
...
@Gold.command(aliases=["kickall", "kickwave"])
async def masskick(ctx):
    await ctx.message.delete()
    users = list(ctx.guild.members)
    for user in users:
        try:
            await user.kick(reason="Gold")
        except:
            pass


@Gold.command(aliases=["spamroles"])
...
@Gold.command(aliases=["masschannels", "masschannel", "ctc"])
...
@Gold.command(aliases=["delchannel"])
...
@Gold.command(aliases=["deleteroles"])
...
@Gold.command(aliases=["purgebans", "unbanall"])
...
async def dm(ctx, user: discord.Member, *, message):
    await ctx.message.delete()
    user = Gold.get_user(user.id)
    if ctx.author.id == Gold.user.id:
        return
    else:
        try:
...
@Gold.command(name='get-color', aliases=['color', 'colour', 'sc', "hexcolor", "rgb"])
...
@Gold.command(aliases=["guildinfo"])
...
@Gold.command(aliases=['slots', 'bet', "slotmachine"])
async def slot(ctx):
    await ctx.message.delete()
    emojis = "🍎🍊🍐🍋🍉🍇🍓🍒"
...
@Gold.command(aliases=['guildpfp', 'serverpfp', 'servericon'])
...
@Gold.command(aliases=['serverbanner'])
...
@Gold.command(name='first-message', aliases=['firstmsg', 'fm', 'firstmessage'])
...
@Gold.command(aliases=["renameserver", "nameserver"])
...
async def prefix(ctx, prefix):
    await ctx.message.delete()
    Gold.command_prefix = str(prefix)
...
@Gold.command(pass_context=True, aliases=["cyclename", "autoname", "autonick", "cycle"])
...
@Gold.command(aliases=["stopcyclename", "cyclestop", "stopautoname", "stopautonick", "stopcycle"])
...
async def acceptfriends(ctx):
    await ctx.message.delete()
    for relationship in Gold.user.relationships:
...
@Gold.command(aliases=["changeregions", "changeregion", "regionschange"])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Gold_dog.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Gold_cat.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Gold_sadcat.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Gold_bird.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Gold_fox.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Gold_anal.gif"))
    except:
        em = discord.Embed()
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Gold_erofeet.png"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Gold_feet.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Gold_hentai.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Gold_boobs.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Gold_tits.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Gold_blowjob.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Gold_neko.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Gold_lesbian.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Gold_cumslut.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Gold_pussy.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Gold_waifu.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Gold_feed.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Gold_tickle.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Gold_slap.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Gold_hug.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Gold_cuddle.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Gold_smug.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Gold_pat.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Gold_kiss.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
...
async def purge(ctx, amount: int):
    await ctx.message.delete()
    async for message in ctx.message.channel.history(limit=amount).filter(lambda m: m.author == Gold.user).map(
            lambda m: m):
        try:
            await message.delete()
        except:
...
async def stream(ctx, *, message):
    ...
    await Gold.change_presence(activity=stream)


@Gold.command(alises=["game"])
async def playing(ctx, *, message):
    await ctx.message.delete()
    game = discord.Game(
        name=message
    )
    await Gold.change_presence(activity=game)


@Gold.command(aliases=["listen"])
async def listening(ctx, *, message):
    await ctx.message.delete()
    await Gold.change_presence(
...
@Gold.command(aliases=["stopstreaming", "stopstatus", "stoplistening", "stopplaying", "stopwatching"])
async def stopactivity(ctx):
    await ctx.message.delete()
    await Gold.change_presence(activity=None, status=discord.Status.dnd)
...
@Gold.command(aliases=["fliptable"])
...
@Gold.command(aliases=["logout"])
async def shutdown(ctx):
    await ctx.message.delete()
    await Gold.logout()


@Gold.command(aliases=["nitrogen"])
async def nitro(ctx):
    await ctx.message.delete()
